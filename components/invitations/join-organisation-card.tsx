...
export type JoinOrganisationCardProps = CardProps & {
  invitation: Invitation;
  organisationName: string;
};

export function JoinOrganisationCard({
  invitation,
  organisationName,
  ...other
}: JoinOrganisationCardProps): React.JSX.Element {
  ...
  const onSubmit: SubmitHandler<JoinOrganisationSchema> = async (values) => {
    if (!canSubmit) {
      return;
    }
    const result = await join(values);
    if (result?.serverError || result?.validationErrors) {
      toast.error("Couldn't join organisation");
      setErrorMessage("Couldn't join organisation");
    }
  };
  return (
